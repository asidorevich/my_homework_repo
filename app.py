...
LOGO_URL = "https://github.com/asidorevich/my_homework_repo/blob/main/logo.PNG"  # ← ТВОЯ ССЫЛКА ЗДЕСЬ

st.markdown(f"""
<div style="text-align:center; padding:60px 0 40px;">
    <img src="{LOGO_URL}?raw=true" width="150" 
         style="border-radius:50%; border: 6px solid white; box-shadow: 0 0 50px rgba(0,212,255,0.4);"
         onerror="this.style.display='none'; this.nextElementSibling.style.display='block';">
    <div style="display:none; width:150px; height:150px; background: linear-gradient(45deg, #ff8c42, #00d4ff); border-radius:50%; margin:20px auto; border: 6px solid white; box-shadow: 0 0 50px rgba(0,212,255,0.4);"></div>
    <h1 style="font-size:5.5rem;">OLYMPUS</h1>
    <p style="font-size:1.6rem; color:#ff8c42; letter-spacing:5px;">2026</p>
</div>
""", unsafe_allow_html=True)
# ←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←
...
if st.session_state.role is None:
    ...
    with c2:
        ...
        role = st.selectbox("Роль", ["Снабжение (закупки)", "Медсестра (списание)"])
        pwd = st.text_input("Пароль", type="password")
        if st.button("ВОЙТИ", use_container_width=True, type="primary"):
            if role == "Снабжение (закупки)" and pwd == "olympus2025":
                st.session_state.role = "snab"; st.rerun()
            elif role == "Медсестра (списание)" and pwd == "med123":
...
# ===================== СНАБЖЕНИЕ =====================
    ...
    # (весь остальной код Снабжения остался без изменений — он у тебя уже идеальный)
    with t1:
        st.subheader("Новые заявки от медсестёр")
        st.markdown("<div class='big-refresh'>", unsafe_allow_html=True)
        if st.button("ОБНОВИТЬ ЗАЯВКИ", key="big_refresh_orders", use_container_width=True, type="primary"):
            load_fresh_data()
    ...
    with t5:
        ...
        load_fresh_data()
        if purchases.empty:
            st.info("Пока нет ни одной закупки в базе")
            st.stop()
